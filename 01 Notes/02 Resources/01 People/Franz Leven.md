---
aliases: fleven
Full Name: Franz Leven
tags: person-note
ping: 
contact:
- email: franz.leven@kickstartds.com
- phone: 
- whatsApp: 
- twitter: 
- linkedin: https://www.linkedin.com/in/franz-leven-b398aa232
- github: https://github.com/fleven-kds
location: Koh Phangan, Thailand
banner: https://images.unsplash.com/photo-1585854467604-cf2080ccef31?crop=entropy&cs=tinysrgb&fit=crop&fm=jpg&h=150&ixid=MnwxfDB8MXxyYW5kb218MHx8YWJzdHJhY3QgdGV4dHVyZXx8fHx8fDE2NzMxMzA5MDE&ixlib=rb-4.0.3&q=80&utm_campaign=api-credit&utm_medium=referral&utm_source=unsplash_source&w=900
---
# Franz Leven

## Details

### Birthday
[Birthday :: 11-17]

#### Reminder
- [ ] [[Franz Leven]] Birthday (11-17) #ann 🔁 every year 📅 2023-11-17

### Company
[Company :: ruhmesmeile GmbH]

#### Role

Designer.

#### Details

Digital native designer with coding chops.

### Notes

No notes yet.

## How We Met

Franz started with ruhmesmeile GmbH for a training period as part of his curriculum. And then he stayed with us till now!

## Meetings

```dataview
TABLE
	full-date as "Full Date"
FROM "01 Notes/02 Resources/03 Meetings"
WHERE contains(Attendees, [[Franz Leven]])
SORT full-date DESC
```
## Projects

```dataview
TABLE
	target as "Target",
	deadline as "Deadline"
FROM "02 Action/02 Projects"
WHERE contains(Team, [[Franz Leven]])
SORT full-date DESC
```