---
tags: meeting-note
full-date: 2023-01-25
week: 2023-W04
month: 2023 - 01-January
year: 2023
area: "[[Experience Engine]]"
project: "[[Experience Engine Sprint 6 (Jan 26 - Feb 12)]]"
---

# 2023-01-25 - ExE Sprint 6 Workshop Commodity 1 of 3

[Date :: January 25, 2023]
[Attendees :: [[Daniel Lehmann]], [[Michael Micke]], [[Ivan Barisic]], [[Lukas Mestel]], [[Elis Westphal]], [[Daniel Ley]], [[Jonas Ulrich]]]
[Length :: `200m`]

---

# Goals / agenda
1. Refine concepts for commodity journey types

# Discussion notes
- Prios:
	- A) General WLP ("classic") case, like Sparstrom. Whole journey, not only the core form flow
	- B) EWE (e.g. street accurate prices)
	- C) Extras

# Action items
No action items.